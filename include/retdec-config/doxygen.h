 ...
 * @brief Doxygen documentation of the retdec_config namespace.
...
@namespace retdec_config A library providing access to configuration of
decompilations.

@section intro_sec Introduction

@c retdec-config library defines many objects used throughout all decompiler's parts
...
This is a short description of all current @c retdec-config library modules.
...
To use entire @c retdec-config library, include only the main @c config.h file
...
in the retdec_config namespace.
...
For example, method:
@code{.cpp}
retdec_config::Function* FunctionContainer::getFunctionByName(const std::string& name);
@endcode
...
std::string createJson()
{
    ...
    config.parameters.selectedRanges.insert( retdec_config::AddressRangeJson(0x1000, 0x2000) );
    ...
    retdec_config::Function fnc("my_function");
    fnc.callingConvention.setIsStdcall();

    retdec_config::Object param("param", retdec_config::Storage::undefined());
    param.type.setLlvmIr("i32");

    retdec_config::Object local("local", retdec_config::Storage::onStack(-20));
...
void parseJson(const std::string& json)
{
    ...
    retdec_config::Function* fnc = config.functions.getFunctionByName("my_function");
    if (fnc)
    {
        out << fnc->getName() << "\n";
    }
