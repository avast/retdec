/**
 * @file include/fileformat/file_format/file_format.h
 * @brief Definition of FileFormat class.
 * @copyright (c) 2017 Avast Software, licensed under the MIT license
 */

#ifndef FILEFORMAT_FILE_FORMAT_FILE_FORMAT_H
#define FILEFORMAT_FILE_FORMAT_FILE_FORMAT_H
 ...
 */
class FileFormat : public tl_cpputils::ByteValueStorage, private tl_cpputils::NonCopyable
{
	private:
		...
		void init();
		void initStream();
		template<typename T> void initFormatArch(T derivedPtr, const retdec_config::Architecture &arch);
		/// @}
	...
	protected:
		...
		tl_cpputils::Maybe<bool> signatureVerified;                    ///< indicates whether the signature is present and also verified
		tl_cpputils::RangeContainer<std::uint64_t> nonDecodableRanges; ///< Address ranges which should not be decoded for instructions.
	...
	public:
		...
		/// @name Other methods
		/// @{
		void initFromConfig(const retdec_config::Config &config);
		void loadStrings();
		...
		/// @{
		bool get1ByteOffset(std::uint64_t offset, std::uint64_t &res, tl_cpputils::Endianness e = tl_cpputils::Endianness::UNKNOWN) const;
		bool get2ByteOffset(std::uint64_t offset, std::uint64_t &res, tl_cpputils::Endianness e = tl_cpputils::Endianness::UNKNOWN) const;
		bool get4ByteOffset(std::uint64_t offset, std::uint64_t &res, tl_cpputils::Endianness e = tl_cpputils::Endianness::UNKNOWN) const;
		bool get8ByteOffset(std::uint64_t offset, std::uint64_t &res, tl_cpputils::Endianness e = tl_cpputils::Endianness::UNKNOWN) const;
		bool get10ByteOffset(std::uint64_t offset, long double &res) const;
		bool getXByteOffset(std::uint64_t offset, std::uint64_t x, std::uint64_t &res, tl_cpputils::Endianness e = tl_cpputils::Endianness::UNKNOWN) const;
		bool getXBytesOffset(std::uint64_t offset, std::uint64_t x, std::vector<std::uint8_t> &res) const;
		bool getWordOffset(std::uint64_t offset, std::uint64_t &res, tl_cpputils::Endianness e = tl_cpputils::Endianness::UNKNOWN) const;
