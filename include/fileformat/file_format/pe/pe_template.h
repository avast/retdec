/**
 * @file include/fileformat/file_format/pe/pe_template.h
 * @brief Template functions for PE files.
 * @copyright (c) 2017 Avast Software, licensed under the MIT license
 */

#ifndef FILEFORMAT_FILE_FORMAT_PE_PE_TEMPLATE_H
#define FILEFORMAT_FILE_FORMAT_PE_PE_TEMPLATE_H
...
template<int bits> bool peSectionType(const PeLib::PeHeaderT<bits> &peHeader, PeCoffSection::Type &secType, unsigned long long sectionIndex)
{
	...
	else if(flags & PeLib::PELIB_IMAGE_SCN_MEM_DISCARDABLE && peSectionName(peHeader, name, sectionIndex) && tl_cpputils::startsWith(name, ".debug_"))
	{
 ...
 */
template<int bits> tl_cpputils::RangeContainer<std::uint64_t> peImportDirectoryOccupiedAddresses(const PeLib::ImportDirectory<bits> &peImports)
{
	tl_cpputils::RangeContainer<std::uint64_t> result;
	for (const auto& addresses : peImports.getOccupiedAddresses())
	{
		try
		{
			result.addRange(tl_cpputils::Range<std::uint64_t>{addresses.first, addresses.second});
		}
		catch (const tl_cpputils::InvalidRangeException&)
		{
			continue;
		}
	}
 ...
 * @return Occupied address ranges
 */
template<int bits> tl_cpputils::RangeContainer<std::uint64_t> peExportDirectoryOccupiedAddresses(const PeLib::ExportDirectoryT<bits> &peExports)
{
	tl_cpputils::RangeContainer<std::uint64_t> result;
 ...
 * @return Occupied address ranges
 */
template<int bits> tl_cpputils::RangeContainer<std::uint64_t> peDebugDirectoryOccupiedAddresses(const PeLib::DebugDirectoryT<bits> &peDebug)
{
	tl_cpputils::RangeContainer<std::uint64_t> result;
 ...
 * @return Occupied address ranges
 */
template<int bits> tl_cpputils::RangeContainer<std::uint64_t> peResourceDirectoryOccupiedAddresses(const PeLib::ResourceDirectoryT<bits> &peResources)
{
	tl_cpputils::RangeContainer<std::uint64_t> result;
