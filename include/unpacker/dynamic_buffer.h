/**
 * @file include/unpacker/dynamic_buffer.h
 * @brief Declaration of class for buffered data mainpulation.
 * @copyright (c) 2017 Avast Software, licensed under the MIT license
 */

#ifndef UNPACKER_DYNAMIC_BUFFER_H
#define UNPACKER_DYNAMIC_BUFFER_H
...
{
public:
	DynamicBuffer(tl_cpputils::Endianness endianness = tl_cpputils::Endianness::LITTLE);
	DynamicBuffer(uint32_t capacity, tl_cpputils::Endianness endianness = tl_cpputils::Endianness::LITTLE);
	DynamicBuffer(const std::vector<uint8_t>& data, tl_cpputils::Endianness endianness = tl_cpputils::Endianness::LITTLE);
	...
	void setEndianness(tl_cpputils::Endianness endianness);
	tl_cpputils::Endianness getEndianness() const;
	 ...
	 */
	template <typename T> T read(uint32_t pos, tl_cpputils::Endianness endianness = tl_cpputils::Endianness::UNKNOWN) const
	{
		...
		if (endianness == tl_cpputils::Endianness::UNKNOWN)
	 ...
	 */
	template <typename T> void write(const T& data, uint32_t pos, tl_cpputils::Endianness endianness = tl_cpputils::Endianness::UNKNOWN)
	{
...
private:
	template <typename T> void writeImpl(const T& data, uint32_t pos, tl_cpputils::Endianness endianness)
	{
		...
		for (uint32_t i = 0; i < bytesToWrite; ++i)
		{
			switch (endianness)
			{
				case tl_cpputils::Endianness::LITTLE:
					_data[pos + i] = (data >> (i << 3)) & 0xFF;
					break;
				case tl_cpputils::Endianness::BIG:
	...
	template <typename T> T readImpl(uint32_t pos, tl_cpputils::Endianness endianness) const
	{
	...
	tl_cpputils::Endianness _endianness;
	uint32_t _capacity;
};
