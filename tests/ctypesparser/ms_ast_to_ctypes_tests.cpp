...
TEST_F(MsCtypesTests, SignednessOfTypesWithKnownSignedness)
{
	...
	func = demangler->demangleFunctionToCtypes("?foo@@YAHH@Z", module, typeWidths, typeSignedness);
	auto int_type = func->getParameter(1).getType();
	EXPECT_TRUE(std::static_pointer_cast<ctypes::IntegralType>(int_type)->isSigned());

	func = demangler->demangleFunctionToCtypes("?foo@@YAHI@Z", module, typeWidths, typeSignedness);
	auto uint_type = func->getParameter(1).getType();
	EXPECT_FALSE(std::static_pointer_cast<ctypes::IntegralType>(uint_type)->isSigned());

	func = demangler->demangleFunctionToCtypes("?foo@@YAHC@Z", module, typeWidths, typeSignedness);
	auto signed_char_type = func->getParameter(1).getType();
	EXPECT_TRUE(std::static_pointer_cast<ctypes::IntegralType>(int_type)->isSigned());

	func = demangler->demangleFunctionToCtypes("?foo@@YAHE@Z", module, typeWidths, typeSignedness);
...
TEST_F(MsCtypesTests, SignednessOfTypesWithSignednessInMap)
{
	...
	func = demangler->demangleFunctionToCtypes("?foo@@YAH_W@Z", module, typeWidths, typeSignednessSignedWchar);
	...
	func = demangler->demangleFunctionToCtypes("?foo@@YAH_W@Z", module, typeWidths, typeSignednessUnsignedWchar);
