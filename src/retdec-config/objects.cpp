/**
 * @file src/retdec-config/objects.cpp
...
{
	auto s = retdec_config::Storage::inRegister(name);
	auto r = retdec_config::Object(name, s);
	r.setRealName(name);
	return insert(r);
}
...
const Object* GlobalVarContainer::getObjectByAddress(
		const tl_cpputils::Address& address) const
{
