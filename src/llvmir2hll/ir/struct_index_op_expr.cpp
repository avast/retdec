...
*/
StructIndexOpExpr::StructIndexOpExpr(Expression* base,
	ConstInt* fieldNumber):
		BinaryOpExpr(Value::ValueKind::StructIndexOpExpr, base, fieldNumber) {}

Type* StructIndexOpExpr::getType() const {
	...
	if (StructType* op1StructType = cast<StructType>(op1Type)) {
		return op1StructType->getTypeOfElement(ucast<const ConstInt>(op2));
	}
...
bool StructIndexOpExpr::isEqualTo(Value* otherValue) const {
	if (StructIndexOpExpr* otherValueStructIndexOpExpr =
...
Value* StructIndexOpExpr::clone() {
	StructIndexOpExpr* structIndexOpExpr(StructIndexOpExpr::create(
...
*/
StructIndexOpExpr* StructIndexOpExpr::create(Expression* base,
		ConstInt* fieldNumber) {
	PRECONDITION_NON_NULL(base);
	PRECONDITION_NON_NULL(fieldNumber);

	StructIndexOpExpr* expr(new StructIndexOpExpr(base, fieldNumber));
...
void StructIndexOpExpr::accept(Visitor *v) {
	v->visit(ucast<StructIndexOpExpr>(this));
}
