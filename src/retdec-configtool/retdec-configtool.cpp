/**
 * @file src/retdec-configtool/retdec-configtool.cpp
...
bool hasEnding(const std::string &str, const std::set<std::string> &suffixes)
{
	return std::any_of(suffixes.begin(), suffixes.end(),
		[&] (const auto &suffix) { return tl_cpputils::endsWith(str, suffix); });
}
...
{
	tl_cpputils::FilesystemPath fsp(dirPath);
	if (!fsp.isDirectory())
	{
		return;
	}
	...
	for (auto f : fsp)
	{
				...
				&& hasEnding(f->getPath(), suffixes))
		{
			auto p = fsp.separator() == '\\'
					? tl_cpputils::replaceAll(f->getPath(), "\\", "/")
					: f->getPath();
			ret.push_back(p);
		}
	}
...
int handleArguments(std::vector<std::string> &args)
{
	...
	catch (const retdec_config::Exception& e)
	{
	...
	else if (args[1] == "--write")
	{
		for (size_t i = 2; i<args.size()-1; i+=2)
		{
			...
			else if (opt == "--entry-point")
			{
				...
				if (tl_cpputils::strToNum(val, n, std::hex))
				{
			...
			{
				config.parameters.selectedRanges.insert( retdec_config::AddressRangeJson(val) );
			}
	...
	{
		retdec_config::Config newConfig;
