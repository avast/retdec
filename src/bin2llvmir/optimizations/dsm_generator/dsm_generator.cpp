...
void DsmGenerator::generateHeader(std::ostream& ret)
{
	...
	ret << ";; Copyright (c) " << tl_cpputils::getCurrentYear()
	    << " Retargetable Decompiler <info@retdec.com>\n";
	ret << ";;\n";
	ret << ";; Decompilation date: "
	    << tl_cpputils::getCurrentDate() << " "
	    << tl_cpputils::getCurrentTime() << "\n";
	ret << ";; Architecture: "
...
std::string DsmGenerator::processInstructionDsm(AsmInstruction& ai)
{
	...
	else if (auto* br = ai.getInstructionFirst<BranchInst>())
	{
		...
		if (ok && trueDestAi.isValid() && trueDestAi != ai)
		{
			...
			{
				tl_cpputils::Address o = trueDestAi.getAddress() - addr;
		...
		const llvm::ConstantDataArray* cda)
{
	...
	{
		tl_cpputils::WideStringType str;
...
std::string DsmGenerator::getFunctionName(retdec_config::Function* f) const
{
