...
BIN2LLVMIR_LLVM_PASSES_ONLY = ['-instcombine', '-tbaa', '-targetlibinfo', '-basicaa', '-domtree', '-simplifycfg', '-domtree', '-early-cse',
...
FILEINFO_EXTERNAL_YARA_EXTRA_CRYPTO_DATABASES = [os.path.join(INSTALL_SHARE_YARA_DIR, 'signsrch', 'signsrch_regex.yara')]
...
DECOMPILER_SH = os.path.join(INSTALL_BIN_DIR, 'retdec-decompiler.sh')
ARCHIVE_DECOMPILER_PY = os.path.join(INSTALL_BIN_DIR, 'retdec_archive_decompiler.py')
SIG_FROM_LIB_PY = os.path.join(INSTALL_BIN_DIR, 'retdec_signature_from_library_creator.py')
UNPACK_PY = os.path.join(INSTALL_BIN_DIR, 'retdec_unpacker.py')
